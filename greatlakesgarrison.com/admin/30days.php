...
group by tkid having max(event_date) < date_sub(curdate(), interval 335 day)
	and max(event_date) > date_sub(curdate(), interval 365 day)
order by event_date

");
?>
<b>Troopers within 30 days of one-year mark of last activity in approved 501st costume:</b><P>
...
<?
while ($row=mysql_fetch_array($in_30_days_sql)) {
	$tkid = $row['tkid'];
